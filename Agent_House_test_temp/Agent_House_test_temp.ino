...
// .iso.org.0dod.internet.mgmt.mib-2.system.sysName (.1.3.6.1.2.1.1.5)
...
//
static char sysTempSensor[] PROGMEM   = "1.3.6.1.2.1.1.8.0";  // read-only  (Integer)
...
static char locContact[40]          = "Joao Neto - Edmar - Kleyton";                            // should be stored/read from EEPROM - read/write (not done for simplicity)
static char locName[20]             = "Agentuino";                              // should be stored/read from EEPROM - read/write (not done for simplicity)
static char locLocation[20]         = "Natal, RN";                        // should be stored/read from EEPROM - read/write (not done for simplicity)
...
if ( (pdu.type == SNMP_PDU_GET || pdu.type == SNMP_PDU_GET_NEXT || pdu.type == SNMP_PDU_SET)
    ...
    else if ( strcmp_P(oid, sysHumSensor ) == 0 ) {
      // handle sysDescr (set/get) requests
      if ( pdu.type == SNMP_PDU_SET ) {
        // response packet from set-request - object is read-only
        pdu.type = SNMP_PDU_RESPONSE;
        ...
        Serial.println(DHT11.temperature);
        Serial.println(DHT11.humidity);
        status = pdu.VALUE.encode(SNMP_SYNTAX_INT, DHT11.humidity);
        pdu.type = SNMP_PDU_RESPONSE;
        pdu.error = status;
      }
      //
      #ifdef DEBUG
        Serial << F("Valor Temperatura ... ") << DHT11.humidity << F(" ") << pdu.VALUE.size << endl;
      #endif
    }
